...
export default function Footer() {
    ...
    return (<footer className='bg-softWhite text-softBlue font-accent sm:text-md text-sm flex-col items-center flex'>
               
            <Image src="/images/logo2.png" alt='logo' width={150} height={0}/>
            ...
            <div className='md:space-x-8 space-x-4'>
                {navLinks.map((link) => (<Link key={link.path} href={link.path} className={`relative pb-1 transition-all duration-300
                            ${pathname === link.path}`}>
                        {link.name}
                    </Link>))}
            </div>
