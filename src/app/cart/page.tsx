...
export default function CartPage() {
  ...
  const updateQuantity = (_id: string, amount: number) => {
    const newCart = cart.map((item) =>
      item._id === _id ? { ...item, quantity: Math.max(1, item.quantity + amount) } : item
    );
    setCart(newCart);
  ...
  return (
    <div className="min-h-screen bg-softWhite p-6 md:p-12 font-body text-softBrown">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-8 space-y-4">
            {cart.map((item) => (
              <div
                key={item._id}
                className="flex items-center justify-between bg-softPink p-4 rounded shadow-sm"
              >
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center border rounded">
                    <button
                      ...
                      className="px-2 py-1 hover:bg-softBrown hover:text-softWhite transition"
                    >
                      -
                    </button>
                  ...
                  <button
                    ...
                    className="text-softCoral hover:text-green font-bold transition"
                  >
                    Remove
                  </button>
                </div>
          ...
          <div className="md:col-span-4 bg-softBlue p-6 rounded shadow-md h-fit">
            ...
            <Link
              href="/checkout"
              className="block text-center bg-softCoral text-softWhite py-3 rounded hover:bg-green transition"
            >
              Proceed to Checkout
            </Link>
          </div>
